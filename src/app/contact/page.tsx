        ...
        <div className="px-site">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 48, alignItems: 'start' }} className="contact-grid-responsive">
            ...
            <Reveal delay={1} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={{ background: 'var(--slate)', borderRadius: 'var(--r-lg)', padding: '28px 26px', position: 'relative', overflow: 'hidden' }}>
                ...
                {[
                  ...
                  <div key={row.label} style={{ display: 'flex', gap: 14, alignItems: 'flex-start', marginBottom: 20, paddingBottom: 20, borderBottom: '1px solid rgba(255,255,255,.07)' }}>
                    <div>
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,.35)', marginBottom: 4 }}>{row.label}</div>
                      <div style={{ fontSize: 15, color: '#fff', fontWeight: 500 }}>{row.value}</div>
                    </div>
                  </div>
                ))}
                <div style={{ borderBottom: 'none', marginBottom: 0, paddingBottom: 0 }}>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,.35)', marginBottom: 4 }}>Status</div>
                  <div style={{ fontSize: 15, color: '#fff', fontWeight: 500, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#34C759', boxShadow: '0 0 6px #34C759', flexShrink: 0, display: 'inline-block' }} />
              ...
              {/* Who we work with */}
              <div style={{ background: 'var(--bg-3)', borderRadius: 'var(--r-lg)', border: '1px solid var(--line-2)', padding: '24px 26px', boxShadow: 'var(--shadow-sm)' }}>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '.14em', textTransform: 'uppercase', color: 'var(--ink-3)', marginBottom: 16 }}>Who we work with</div>
                {['Mining companies', 'Governments & regulators', 'Investors & funds'].map(c => (
                  <div key={c} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid var(--line-2)', fontSize: 14, color: 'var(--ink-2)' }}>
