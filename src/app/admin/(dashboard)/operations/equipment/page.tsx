  ...
  return (
    <div className="content content-pad">
      <div style={{ marginBottom: 20 }}>
        ...
        <p className="page-sub">Equipment registry and 30-day utilization from field-logged fuel/breakdown/meter events.</p>
      </div>

      <div className="card" style={{ maxWidth: 520, marginBottom: 20 }}>
        <div className="section-title">Add equipment</div>
        <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
          <input className="input input-sm" style={{ flex: 2 }} placeholder="Name *" value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === 'Enter' && add()} />
          <input className="input input-sm" style={{ flex: 1 }} placeholder="Type (e.g. excavator)" value={type} onChange={e => setType(e.target.value)} onKeyDown={e => e.key === 'Enter' && add()} />
        </div>
      ...
      <div className="card">
        <div className="section-title">Utilization (30-day)</div>
        {loading ? <div style={{ fontSize: 12, color: 'var(--label-4)', padding: 16 }}>Loading…</div> : (
