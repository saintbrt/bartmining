...
type MapStage = 'cleaning' | 'analysis' | 'outputs'

const STAGES: { key: MapStage; label: string }[] = [
  { key: 'cleaning',  label: 'Cleaning' },
  { key: 'analysis',  label: 'Analysis' },
  { key: 'outputs',   label: 'Outputs' },
]

function ProgressStrip({ active, done }: { active: MapStage; done: Set<MapStage> }) {
  return (
        ...
        return (
          <div key={s.key} style={{ display: 'flex', alignItems: 'center', flex: i < STAGES.length - 1 ? 1 : undefined }}>
            <span style={{
              ...
              color: isDone ? 'var(--green)' : isActive ? 'var(--gold)' : 'var(--label-4)',
              whiteSpace: 'nowrap',
            }}>
              {isDone ? '✓ ' : ''}{s.label}
            </span>
            {i < STAGES.length - 1 && (
              <div style={{ flex: 1, height: 1, background: isDone ? 'var(--green)' : 'var(--sep)', margin: '0 10px', opacity: 0.5 }} />
            )}
          </div>
        )
      })}
...
function OutputsPanel() {
  ...
  if (!ctx || !ctx.project || !ctx.user) return null
  ...
  async function buildPreview() {
    ...
    if (!intInv.from || !intInv.to) { notify('warn', `"${interval.name}" has no From/To depth columns mapped.`); return }
  ...
  function saveOutput(rows: TableRow[], name: string) {
    ...
    if (o) notify('success', `Output "${o.name}" saved — ${o.row_count.toLocaleString()} rows.`)
    setPreview(null)
    refresh()
  }
  ...
  return (
    <div>
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        <p style={{ fontSize: 12, color: 'var(--label-3)', marginBottom: 14 }}>One row per collar hole, joined with per-hole grade statistics from the chosen interval file.</p>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>Build PPM summary</div>
        <p style={{ fontSize: 12, color: 'var(--label-3)', marginBottom: 14 }}>One row per hole, peak-grade interval, pooled across the cleaned files you select.</p>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>Save a file as output</div>
      ...
      {preview && (
        <div className="card" style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            <div style={{ fontSize: 13, fontWeight: 600, flex: 1 }}>Preview — {preview.length.toLocaleString()} rows (showing first 20)</div>
      ...
      {outputs.length === 0 ? (
        <div style={{ padding: 40, textAlign: 'center', color: 'var(--label-4)', fontSize: 13 }}>No outputs yet.</div>
      ) : (
          ...
          {outputs.map(o => (
            <div key={o.id} className="card" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <button className="btn-icon" style={{ fontSize: 11, color: 'var(--red)' }} title="Delete"
                onClick={() => { if (window.confirm(`Delete output "${o.name}"?`)) { DB.deleteOutput(o.id, project.id); notify('info', `Deleted output "${o.name}".`); refresh() } }}>✕</button>
            </div>
          ))}
        </div>
      )}
...
export default function MapDataPage() {
  const ctx = useAppContext()
  const [activeStage, setActiveStage] = useState<MapStage>('cleaning')
  ...
  const doneStagess = new Set<MapStage>([
    ...(ss.cleaning === 'done' ? ['cleaning' as MapStage] : []),
    ...(ss.analysis === 'done' ? ['analysis' as MapStage] : []),
  ])

  function handleApprove() {
    if (activeStage === 'cleaning') { approveStage('cleaning'); setActiveStage('analysis') }
    else if (activeStage === 'analysis') { approveStage('analysis'); setActiveStage('outputs') }
  }

  return (
    <div className="content content-pad">
      <ProgressStrip active={activeStage} done={doneStagess} />

      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {STAGES.map(s => (
          <button key={s.key}
            className={`btn btn-sm ${activeStage === s.key ? 'btn-primary' : 'btn-secondary'}`}
      ...
      {activeStage === 'outputs' && <OutputsPanel />}
    </div>
  )
}
