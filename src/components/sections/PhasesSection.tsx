        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20 }} className="phases-grid-responsive">
          {PHASES.map((p, i) => (
            <Reveal key={p.k} delay={i} style={{
              ...
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '.18em', textTransform: 'uppercase', color: 'var(--gold-deep)', marginBottom: 14 }}>{p.k}</div>
              <h3 style={{ fontSize: 20, marginBottom: 16 }}>{p.t}</h3>
              <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 20px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                {p.items.map(item => (
                  <li key={item} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, fontSize: 14, color: 'var(--ink-2)' }}>
                    <span style={{ color: 'var(--gold)', marginTop: 3, flexShrink: 0 }}>&#8250;</span>
                    {item}
                  </li>
                ))}
              </ul>
              <div style={{ paddingTop: 16, borderTop: '1px solid var(--line-2)', fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '.06em', color: 'var(--ink-3)', lineHeight: 1.6 }}>
                {p.deliver}
              </div>
            </Reveal>
          ))}
