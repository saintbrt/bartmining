...
/* ── Gate cookie sync ─────────────────────────────────────────
   The Vercel Edge Middleware guarding /admin/* verifies the
   `gp-auth` cookie (a signed Supabase JWT). Keep it in lock-step
   with the live session so it never goes stale mid-use. */
function setAuthCookie(token, expiresInSec){
  const maxAge = Math.max(60, Math.min(expiresInSec || 3600, 3600));
  document.cookie = "gp-auth=" + token + "; Path=/admin; Max-Age=" + maxAge + "; SameSite=Lax; Secure";
}
function clearAuthCookie(){
  document.cookie = "gp-auth=; Path=/admin; Max-Age=0; SameSite=Lax; Secure";
}
if (sb) {
...
const DB = {
  ...
  async restoreSession(){
    ...
    // Live + no session → bounce to the gated login (middleware also enforces server-side)
    if(!_c.user && SB_READY && location.pathname.startsWith("/admin")){ location.replace("/admin/login"); }
    return _c.user;
  },
  signOut(){
    ...
    if(SB_READY && location.pathname.startsWith("/admin")){ location.replace("/admin/login"); }
  },
