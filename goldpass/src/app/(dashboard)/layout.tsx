...
import type { Project, TableMeta } from '@/lib/db'
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  function approveStage(stage: keyof StageStatus) {
    ...
    setStageStatus(prev => {
      ...
      if (idx >= 0 && idx < order.length - 1) router.push('/' + order[idx + 1])
  ...
  function handleSignOut() { DB.signOut(); router.push('/login') }

  const curView = pathname.replace('/', '').split('/')[0] || 'dashboard'
